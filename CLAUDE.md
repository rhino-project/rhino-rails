# Rhino Rails Server — Development Guide

This is **Rhino**, a Rails gem that auto-generates fully-featured REST APIs from model definitions. It is a Ruby library (not an application) — you are editing the framework itself, not a project that uses it.
...
├── concerns/
│   ├── has_rhino.rb           # Query builder DSL (rhino_filters, rhino_sorts, etc.)
...
├── models/
│   ├── rhino_model.rb         # Base model (pre-includes core concerns)
...
| 6 | **Validation** (per-model, per-action **request classes** — `{Model}StoreRequest` / `{Model}UpdateRequest` in `app/requests/`, autoloaded by Zeitwerk, or registered explicitly with `config.model :slug, "Klass", store_request:/update_request:`. They extend `Rhino::ResourceRequest` (ActiveModel attributes + validations) and receive `user`, `organization`, `route_group`, `action`, `record`, `input`; `authorize?` false → 403 with the policy-denial body, failing rules → 422 `{errors:}`. The declared attributes present in the prepared input ARE the write payload, so an undeclared field is dropped. Resolved per request, never memoized (Zeitwerk reloading); an unresolvable EXPLICIT registration (missing class or wrong superclass) raises `Rhino::ConfigurationError`, while a CONVENTION hit with the wrong superclass is logged and ignored so pre-existing apps that own a same-named class keep working. Applies to store/update and to each `POST /nested` operation. **Deprecated:** model-level ActiveModel validations with the `allow_nil: true` convention, used only when no request class exists for that model + action) | `resource_request.rb`, `configuration_error.rb`, `resources_controller.rb` (`request_class_for`, `run_resource_request`), `configuration.rb`, `has_validation.rb` (legacy path + `rhino_validate_foreign_keys`) |
...
| 12a | **Computed Attributes** (collection aggregates via `GET /{resource}/computed?attributes=`, opt-in per-row values via `?computed_attributes=` on index/show/trashed; declared in `rhino_collection_computed_attributes` / `rhino_record_computed_attributes`. Attributes may declare PARAMETERS — `{ params: [...], optional: [...], with: ->(...) }` — supplied as `?attributes[name][param]=value`, bound by name in declared order after `user`; a parameterised entry is called strictly with `*args` while a parameterless one keeps the tolerant arity 0/1/2 branch; a bare `/computed` skips required-parameter attributes rather than 403ing. Merged before the policy blacklist/whitelist; undeclared and policy-denied return the same 403) | `resources_controller.rb` (`computed`, `resolve_requested_collection_attributes`, `resolve_requested_computed_attributes`), `computed_attribute_spec.rb`, `argument_binder.rb`, `concerns/hidable_columns.rb`, `invalid_computed_attribute_arguments_error.rb` |
...
| 21 | **Hidden Columns** (base + model-level `rhino_additional_hidden` + policy-level dynamic hiding + `as_rhino_json` for computed attributes) | `hidable_columns.rb` |
| 22 | **Auto-Scope Discovery** (naming convention: `ModelScopes::{Model}Scope`) | `has_auto_scope.rb` |
| 23 | **UUID Primary Keys** | `has_uuid.rb` |
| 24 | **Middleware Support** (global per model `rhino_middleware` + per action `rhino_middleware_actions`) | `resources_controller.rb` |
| 25 | **Action Exclusion** (`rhino_except_actions` to disable specific routes) | `routes.rb` |
...
| 29 | **Named Scopes** (`?scope=availableForDrivers`, or `?scope[name][param]=value` with parameters declared in `rhino_scopes`, up to `config.max_scopes_per_request` (default 3) per request; whitelisted via `rhino_scopes` + `rhino_default_scope` + policy `permitted_scopes`; camelCase→underscored; unknown, denied or badly-argued → 403; `index`/`trashed` only, `show` stays unscoped) | `query_builder.rb`, `scope_spec.rb`, `argument_binder.rb`, `concerns/has_rhino.rb`, `resources_controller.rb`, `scope_not_allowed_error.rb`, `invalid_scope_arguments_error.rb` |
...
1. **Rhino Docs** — The Docusaurus documentation site at `../rhino-docs/docs/rails/`:
...
2. **Rhino Skill File** — The AI reference file at `../rhino-docs/static/skills/rails/SKILL.md`:
...
Rhino exists in three stacks (Laravel, AdonisJS, Rails). When adding a feature to this Rails version:

- Check if the same feature should be added to `../rhino-server/` (Laravel) and `../rhino-adonis-server/` (AdonisJS)
...
- New concerns go in `lib/rhino/concerns/`, new commands in `lib/rhino/commands/`
- Configuration uses DSL methods on `Rhino.configure` block
...
- Always use the org from `RequestStore[:rhino_organization]` (set by middleware), never from params
